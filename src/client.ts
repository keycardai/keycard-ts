...
import { PolicyBundle } from './resources/policy-bundle';
import {
  Organization,
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['KEYCARD_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['KEYCARD_API_LOG'] or 'warn' if it isn't set.
   */
...
export class KeycardAPI {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? KeycardAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('KEYCARD_API_CUSTOM_HEADERS');
    if (customHeadersEnv) {
...
export declare namespace KeycardAPI {
  ...
  export { PolicyBundle as PolicyBundle };
}
