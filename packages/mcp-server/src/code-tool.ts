...
import { ClientOptions } from '@keycardai/api';

const prompt = `Runs JavaScript code to interact with the Keycard API API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    KEYCARD_API_API_KEY: readEnv('KEYCARD_API_API_KEY') ?? client.apiKey ?? undefined,
    KEYCARD_API_CLIENT_ID: readEnv('KEYCARD_API_CLIENT_ID') ?? client.clientID ?? undefined,
    KEYCARD_API_CLIENT_SECRET: readEnv('KEYCARD_API_CLIENT_SECRET') ?? client.clientSecret ?? undefined,
    KEYCARD_API_BASE_URL: readEnv('KEYCARD_API_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@keycardai/api';
