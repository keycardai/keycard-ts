...
describe('resource ssoConnection', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.organizations.ssoConnection.retrieve('x', { expand: ['permissions'], 'X-Client-Request-ID': '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('disable: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.organizations.ssoConnection.disable('x', { 'X-Client-Request-ID': '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(KeycardAPI.NotFoundError);
  });
  ...
  test.skip('enable: only required params', async () => {
    const responsePromise = client.organizations.ssoConnection.enable('x', { client_id: 'client_id', identifier: 'x' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
